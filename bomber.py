...
def do_zip_update():
    ...
    if success:
        ...
        mesgdcrt.WarningMessage("Grab The Latest one From https://github.com/Whitedevil00117/goat.git")
...
def check_for_updates():
    ...
    fver = requests.get("https://raw.githubusercontent.com/Whitedevil00117/goat/master/.version").text.strip()
    if fver != __VERSION__:
        mesgdcrt.WarningMessage("An update is available")
        mesgdcrt.GeneralMessage("Starting update...")
        update()
    else:
        mesgdcrt.SuccessMessage("Goat is up-to-date")
        mesgdcrt.GeneralMessage("Starting Goat")

def notifyen():
    try:
        noti = requests.get("https://raw.githubusercontent.com/Whitedevil00117/goat/master/.notify").text.upper().strip()
        if len(noti) > 10:
...
def pretty_print(cc,target,success,failed):
    ...
    mesgdcrt.SuccessMessage("use a proxy connection to avoid detection")
