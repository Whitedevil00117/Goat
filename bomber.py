...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("Goat is the latest version")
        mesgdcrt.GeneralMessage("Running...")
    else:
        mesgdcrt.FailureMessage("Unable to update Goat.")
        mesgdcrt.WarningMessage("wait for the latest veraion to release @ github.com/Whitedevil00117")
...
def do_git_update():
    ...
    if success:
        ...
        mesgdcrt.GeneralMessage("Then then run again:")
        print("pin")
...
def check_for_updates():
    ...
    if fver != __VERSION__:
        mesgdcrt.WarningMessage("Locating shell")
        mesgdcrt.GeneralMessage("Starting python shell...")
        update()
    else:
        mesgdcrt.SuccessMessage("Pin")
        mesgdcrt.GeneralMessage("Starting Goat")
...
def pretty_print(cc,target,success,failed):
    ...
    mesgdcrt.SectionMessage("Bombing is in progress ⚙️")
...
if sys.version_info[0]!=3:
    mesgdcrt.FailureMessage("Goat will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['WhiteDevil_','Lucifer']

ALL_COLORS = [Fore.GREEN, Fore.RED, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN, Fore.WHITE]
RESET_ALL = Style.RESET_ALL

description="""Goat - The ultimate sms bomber

Goat can be used for many purposes which incudes - 
...
parser = argparse.ArgumentParser(description=description,epilog='Coded by WhiteDevil_ !!!')
parser.add_argument("-sms","--sms", action="store_true",help="start Goat with SMS Bomb mode")
parser.add_argument("-call","--call", action="store_true",help="start Goat with CALL Bomb mode")
parser.add_argument("-mail","--mail", action="store_true",help="start Goat with MAIL Bomb mode")
parser.add_argument("-u","--update", action="store_true",help="update Goat")
parser.add_argument("-c","--contributors", action="store_true",help="show current Goat contributors")
parser.add_argument("-v","--version", action="store_true",help="show current Goat version")
