...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing Goat'
    else
        figlet Goat
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mWhitedevil_ \e[0m"
    else
        echo -e "\e[1;34mCreated By \e[1;34m"
        toilet -f mono12 -F border WhiteDev
    fi
    echo -e "\e[1;34m For Any Queries Mail Me!!!\e[0m"
    echo -e "\e[1;32m           Mail: whitedevil00117@gmail.com \e[0m"
    echo -e "\e[4;32m  pin \e[0m"
    echo " "
...
if [ -f .update ];then
    ...
    echo .
    install_deps
    echo This Script Was Made By WhiteDevil_ > .update
